...
describe('SettingPanel', () => {
  ...
  it('starts moving section copy to keyed i18n entries', async () => {
    ...
    expect(source).toContain("import { buildBaseSettingSections } from '@/components/setting-panel-sections'")
    expect(source).toContain('v-for="section in baseSections"')
    expect(source).toContain('v-for="field in section.fields"')
