...
describe('SummaryDetailSection', () => {
  ...
  it('starts moving summary detail chrome and ai empty states to keyed i18n entries', async () => {
    ...
    expect(source).toContain("from '@/components/summary-detail-ai-inbox'")
    expect(source).toContain("buildAiInboxHistoryTooltip")
    expect(source).toContain("buildSuggestionCalloutItems")
    expect(source).toContain("hasSuggestionCallout")
    expect(source).toContain("resolveAiInboxTypeLabel")
    expect(source).toContain("resolveAiInboxActionTargetsFromSummaryDetail")
